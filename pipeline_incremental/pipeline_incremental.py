...
def load_state():
    if not STATE_FILE.exists():
        return {"processed_doc_ids": [], "processed_chunk_ids": []}
...
def clean_new_docs():
    ...
    new_units = [u for u in units if u["doc_id"] not in state["processed_doc_ids"]]
...
def chunk_new_docs(new_doc_ids):
    ...
    new_units = [u for u in cleaned if u["doc_id"] in new_doc_ids]
...
def embed_new_chunks(new_chunk_ids):
    ...
    new_chunks = [c for c in chunks if c["chunk_id"] in new_chunk_ids]
